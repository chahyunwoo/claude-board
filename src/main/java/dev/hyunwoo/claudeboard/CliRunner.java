...
/**
 * {@code --json} 모드. 웹 서버 없이 한 번 수집해 표준출력에 찍는다. docs/05-검증.md 의 1~3번 검증이 이 출력을 쓴다.
 *
 * <p>수집 실패로 예외가 나도 {@code errors} 가 실린 JSON 을 내보내는 것이 목표다 — 조용히 죽으면 "세션이 없다"와 "읽지 못했다"가 구별되지 않는다.
 */
