...
/**
 * 소스 A — {@code claude agents --json} 을 실행해 살아있는 세션 목록을 얻는다. docs/01-데이터.md "소스 A".
 *
...
public final class AgentsReader {
    ...
    /**
     * 살아있는 세션 목록. 실행에 실패하면 예외를 던진다 — 호출부가 {@code errors} 로 노출해야 한다. 빈 목록으로 삼키면 "세션이 없다"와 "읽지 못했다"가
     * 구별되지 않는다.
     */
    ...
    private String runCommand() throws IOException {
        ...
        if (process.exitValue() != 0) {
            throw new IOException(
                    "agents 실행 실패 (exit=" + process.exitValue() + "): " + stderr.strip());
        }
        return stdout;
    }
     ...
     *
     * <p>{@code status} 는 {@code "busy"} 일 때만 존재한다 — 없다고 유휴로 간주하지 않는다. 상태 판별은 소스 B(세션 기록)로 한다. 실측에서
     * status 가 없는 항목이 실제로 존재했다.
     */
