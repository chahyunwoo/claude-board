 ...
 *
 * <p><b>조립만 여기서 한다.</b> {@code collect/} 자체는 Spring 을 모르는 순수 자바로 두어 단위 테스트가 빠르고 다른 곳으로 옮겨도 그대로 가게
 * 한다. docs/02-백엔드.md.
 *
 * <p>설정값은 전부 {@link BoardProperties} 에서 온다 — 코드에 상수를 따로 두면 설정을 바꿔도 아무 일이 안 일어나는 죽은 설정이 생긴다.
 */
@Configuration
