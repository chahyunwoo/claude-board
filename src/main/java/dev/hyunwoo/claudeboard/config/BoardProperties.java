 ...
 *
 * <p>기본값을 여기에 둔 이유는 <b>설정이 죽어 있는 것을 막기 위해서</b>다 — 코드에 상수를 따로 두고 {@code max(상수, 설정)} 처럼 쓰면 상수보다 작은
 * 설정이 조용히 무시된다. 값은 이 한 곳에서만 온다.
 *
...
public record BoardProperties(
    ...
    public BoardProperties {
        ...
        transcriptMaxBytes =
                transcriptMaxBytes != null ? transcriptMaxBytes : DataSize.ofKilobytes(512);
    }
}
