...
/**
 * SSE 연결 수명. <b>emitter 누수는 예외를 내지 않고 조용히 쌓이므로</b> 구독자 수를 직접 세어서 확인한다. docs/05-검증.md 5번 "SSE 연결이
 * 끊겼다 붙을 때".
 *
 * <p>실제 HTTP 로 붙였다 끊는다 — 컨트롤러 메서드를 직접 부르면 서블릿 컨테이너가 {@code onCompletion}/{@code onError} 를 부르는 경로를
 * 타지 않아, <b>정리가 배선되지 않았어도 통과해 버린다.</b>
 */
@SpringBootTest(
    ...
    void 연결하면_즉시_현재_상태를_받고_끊으면_구독이_정리된다() throws Exception {
        ...
        try (InputStream in = conn.getInputStream();
            ...
            await().atMost(ofSeconds(5))
                    .untilAsserted(
                            () -> assertThat(boardService.listenerCount()).isEqualTo(before + 1));
        }
        conn.disconnect();
